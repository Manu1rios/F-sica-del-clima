#!/usr/bin/env python3
"""
Download daily NASA POWER data for LETICIA using the POWER API.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download NASA POWER daily temperature and humidity for LETICIA, AMAZONAS")
    
    # Coordenadas de LETICIA, AMAZONAS
    parser.add_argument("--lat", type=float, default=-4.21, help="Latitude for LETICIA (AMAZONAS)")
    parser.add_argument("--lon", type=float, default=-69.94, help="Longitude for LETICIA (AMAZONAS)")
    ...
    out_file = out_dir / f"nasa_power_leticia_temp_humidity_{args.start}_{args.end}_daily.csv"

    print("Requesting NASA POWER (temperature & humidity) for LETICIA, AMAZONAS...")
    print(url)
